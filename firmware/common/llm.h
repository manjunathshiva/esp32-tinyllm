...
// ---- sliding context --------------------------------------------------------
// Generation normally stops dead at seq_len positions. To continue, drop the
// oldest `drop` positions and slide the rest down, so the model keeps a rolling
// window of recent context.
//
// The catch is that cached keys already have RoPE applied at their ORIGINAL
// absolute position, so after sliding they would claim positions they no longer
// occupy. Re-priming the prompt would fix that and costs one full forward pass
// per carried token -- seconds of stall. Instead: RoPE is a rotation by
// (pos * freq) and rotations compose, so re-basing a key from p to p-drop is a
// single extra rotation by -(drop * freq), applied once per cached key. Values
// are untouched; RoPE only ever applies to q and k.
//
// Positions stay inside [0, seq_len), so nothing extrapolates beyond what the
// model was trained on -- and mid-document windows are exactly what training
// sampled, so this stays in distribution.
static void llm_kv_slide(Model *m, Scratch *s, int drop, int filled) {
  int D = m->c.dim, L = m->c.n_layers, S = m->c.seq_len;
  int H = m->c.n_heads, Dh = D / H;
  int keep = filled - drop;
  if (drop <= 0 || keep <= 0) return;

  float rc[64], rs[64];
  int half = Dh / 2;
  if (half > 64) return;  // unsupported head width; caller keeps the hard stop
  for (int i = 0; i < half; i++) {
    float freq = powf(m->c.rope_theta, -2.f * i / Dh);
    float ang = (float)drop * freq;
    rc[i] = cosf(ang);
    rs[i] = -sinf(ang);   // rotate by -drop*freq
  }

  for (int l = 0; l < L; l++) {
    float *kc = s->kcache + (size_t)l * S * D;
    float *vc = s->vcache + (size_t)l * S * D;
    memmove(kc, kc + (size_t)drop * D, (size_t)keep * D * sizeof(float));
    memmove(vc, vc + (size_t)drop * D, (size_t)keep * D * sizeof(float));
    for (int t = 0; t < keep; t++) {
      for (int hh = 0; hh < H; hh++) {
        float *kh = kc + (size_t)t * D + hh * Dh;
        for (int i = 0; i < half; i++) {
          float c = rc[i], sn = rs[i];
          float k1 = kh[i], k2 = kh[i + half];
          kh[i] = k1 * c - k2 * sn;
          kh[i + half] = k2 * c + k1 * sn;
        }
      }
    }
  }
...
// ---- sampling ---------------------------------------------------------------
// Mirrors src/model.py's generate(): scale by temperature, keep the top_k
// logits, softmax over just those, sample. Greedy argmax is a poor default here
// -- it makes this model repeat a sentence verbatim once it reaches a confident
// loop, which is a property of the decode rule, not the weights.

static inline uint32_t llm_rand(uint32_t *state) {
  uint32_t x = *state;
  x ^= x << 13; x ^= x >> 17; x ^= x << 5;
  return (*state = x);
}

// Sample one token from logits[0:n]. temperature <= 0 means greedy; top_k <= 0
// or >= n means no truncation.
static int llm_sample(const float *logits, int n, float temperature,
                      int top_k, uint32_t *rng) {
  if (temperature <= 0.f) {
    int best = 0;
  ...
  if (top_k > 256) top_k = 256;  // bounds the fixed scratch below

  // Selection of the top_k logits by insertion. The `<= kth` early-out makes
  // the common case one comparison per candidate.
  static float vals[256];  // static, not stack: the ESP32 task stack is 8KB
  static int idx[256];
  int filled = 0;
  for (int i = 0; i < n; i++) {
    float v = logits[i];
    if (filled == top_k && v <= vals[filled - 1]) continue;
    int p = filled < top_k ? filled : top_k - 1;
  ...
  // Softmax over the survivors, in temperature-scaled space.
  float maxv = vals[0], sum = 0.f;
  for (int i = 0; i < filled; i++) {
    vals[i] = expf((vals[i] - maxv) / temperature);
    sum += vals[i];
  }

  // Inverse-CDF sample.
  float r = (float)(llm_rand(rng) >> 8) / 16777216.0f * sum;
  float acc = 0.f;
  for (int i = 0; i < filled; i++) {
    acc += vals[i];
